...
#+BEGIN_SRC python
  def make_slit_wcs(db, slit_coords, imhdu, spechdu):
      # Input WCS from image and spectrum
      wim = WCS(imhdu)
      wspec = WCS(spechdu)

      # For original spectrum, the wavelength and slit axes 0-based, but
      # in FITS axis order instead of python access order, since that is
      # the way that that the WCS object likes to do it'
      orig_wavaxis = db['saxis'] - 1
      orig_slitaxis = 2 - db['saxis']
      # New shiny output WCS, constructed from scratch
      w = WCS(naxis=3)

    
      w.wcs.crpix = [1, 1, 1]
      w.wcs.cdelt = [ds, ds, dwav]
      w.wcs.crval = [RA0, Dec0, wav0

      dRA = np.diff(slit_coords['RA'])
      dDec = np.diff(slit_coords['Dec'])
#+END_SRC
...
#+BEGIN_SRC python
  for row in tab:
      ...
      hahdu.data = (hahdu.data - row['zero'])/row['norm']
      niihdu.data = (niihdu.data - row['zero'])/row['norm']

      # Regularize spectral data so that wavelength is x and pos is y
      if row['saxis'] == 1:
          hahdu.data = hahdu.data.T
          niihdu.data = niihdu.data.T
      # Apply polynomial correction along slit
      hahdu.data /= ratio[:, None]
      niihdu.data /= ratio[:, None]

      # Create the WCS object for the calibrated slit spectra
      wslit = make_slit_wcs(row, slit_coords, imhdu, spechdu)
      update_header(hahdu.header, row, slit_coords)
#+END_SRC
