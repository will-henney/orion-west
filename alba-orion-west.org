...
* TODO [1/2] Final tweaks to the zero level - fit 2D Chebyshev
:LOGBOOK:
...
+ [X] First a pilot program that works on the calibrated spectra
  + 
+ [ ] If that works, then incoroprate it directly into the calibration process
...
#+BEGIN_SRC sh :results silent
  ...
  for vrange in $farblueranges; do
      ...
      python multibin-combine.py new-slits-nii$vrange 300
  done
#+END_SRC
...
#+BEGIN_SRC sh :results silent
  ...
  for vrange in $nearredranges; do
      python multibin-combine.py new-slits-ha$vrange 1e4
      python multibin-combine.py new-slits-nii$vrange 3e3
  done
#+END_SRC
...
#+BEGIN_SRC sh :results silent
  ...
  for vrange in $farredranges; do
      python multibin-combine.py new-slits-ha$vrange 3000
